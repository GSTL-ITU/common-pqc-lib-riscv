...
*
* NOTE (RISC-V bare-metal port): Two changes were made to the original
* reference implementation so it links/runs on a bare-metal target with
* no heap syscalls (_sbrk) and no guaranteed <memory.h>:
*   1. <memory.h> -> <string.h>  (memcpy/memcmp/memset live there)
*   2. malloc/free in aes_encrypt_ccm / aes_decrypt_ccm replaced with a
*      stack-allocated VLA of the same size. No functional change for
*      any caller; just avoids a heap dependency.
*********************************************************************/
                    ...
                    WORD mac_len, const BYTE key_str[], int keysize)
{
	...
	// NOTE: stack-allocated (VLA) instead of malloc() - no heap/_sbrk
	// dependency needed on bare metal. Round both payload and associated
	// data up a block size and add an extra block, same sizing as before.
	BYTE buf[payload_len + assoc_len + 48];
                    ...
                    WORD mac_len, int *mac_auth, const BYTE key_str[], int keysize)
{
	...
	// NOTE: stack-allocated (VLA) instead of malloc() - see aes_encrypt_ccm.
	BYTE buf[assoc_len + ciphertext_len /*ciphertext_len = plaintext_len + mac_len*/ + 48];
