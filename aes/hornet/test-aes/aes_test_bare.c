/*********************************************************************
* Filename:   aes_test_bare.c
* Based on:   aes_test.c by Brad Conte (brad AT bradconte.com)
* Details:    Known-answer tests for the AES implementation, stripped
*             of stdio/printf/main() so it can run on bare-metal RISC-V.
*             Only aes_ecb_test/aes_cbc_test/aes_ctr_test/aes_ccm_test
*             (and the aggregate aes_test) are kept - call these from
*             your own main() and report pass/fail however you like
*             (debug register, UART, GPIO, etc).
*********************************************************************/
